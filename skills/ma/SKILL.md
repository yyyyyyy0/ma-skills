...
0. 入力の充足を確認する
   不明な項目は合理的な仮定を置いて進め、仮定した内容を出力に明記すること。
   「主要タスク」が推測不可能な場合のみ、ユーザーに確認する。
1. エンティティ・状態・関係を整理する
2. ユーザーが認識すべき認知順序を定義する
3. その順序を支える最小の視覚システムを決める
4. コントラストと余白のルールを先に定義する
5. 実装方針を出す
   以下の項目のうち、対象スコープに関連するものを含めること（全項目を書く必要はない）:
   - コンポーネントの分割案（何が独立コンポーネントか）
   - 状態管理の方針（どの状態をどこで持つか）
...
- `references/touch.md`（モバイル・タッチが対象の場合）
- `references/optical.md`（知覚補正が関わる場合）
...
このスキルの出力は、依頼スコープに応じた Tier で返すこと。

### 出力スコープ

| Tier | 対象スコープ | 必須セクション |
|------|-------------|---------------|
| **Full** | 画面・ページ全体の設計 | 全6セクション |
| **Focused** | コンポーネントまたはセクションの局所調整 | 目的、実装上の構成案、デザイン根拠 |
| **Minimal** | 単一要素の調整（ボタン、余白、色、テキスト等） | 実装上の構成案、デザイン根拠 |

#### Tier 判定ロジック

- 画面またはページ全体 → **Full**
- セクション単位でも情報構造・状態・CTA の見直しを含む → **Full**
...
- 主要状態と主要な状態遷移
- CTA とその優先順位
- 実装上の構成案（最小項目は Workflow Step 5 を参照）
- デザイン根拠
...
決定された Tier の必須セクションはすべて返すこと。Tier 内の省略は禁止する。
すべての Tier で哲学だけで終わらず、必ず実装指向の構成案まで含めること。

### Tier 適用例

- 「分析ダッシュボードを設計して」→ Full（画面全体の設計）
...
スキルの育て方は `references/growing.md` を参照。
